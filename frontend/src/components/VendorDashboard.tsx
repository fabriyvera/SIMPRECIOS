...
export function VendorDashboard({
  ...
  const handleSavePrice = async (productId: string, productName: string) => {
    ...
    if (!isNaN(price) && price > 0) {
      try {
        ...
        await pricesAPI.updatePrice(bodyPayload, headers); // 👈

        onUpdatePrice(productId, price);
        setLocalProducts(prev => prev.map(p => p.id === productId ? { ...p, price } : p));
  ...
  const handleDeleteProductClick = async (productId: string, productName: string) => {
    try {
      ...
      const headers = await getAuthHeaders(); // 👈
      await pricesAPI.deletePrice(market.id, String(productoIdFinal), headers); // 👈
  ...
  const calculatePriceAlert = (product: any) => {
    ...
    if (nameLimpio.includes("pollo entero") || nameLimpio.includes("pollo kilo")) refPrice = 16.50;
    else if (nameLimpio.includes("pechuga de pollo") || nameLimpio.includes("pechuga")) refPrice = 26.00;
    else if (nameLimpio.includes("pierna de pollo") || nameLimpio.includes("pierna") || nameLimpio.includes("muslo")) refPrice = 19.00;
    else if (nameLimpio.includes("alitas de pollo") || nameLimpio.includes("alitas")) refPrice = 17.00;
    else if (nameLimpio.includes("carne molida") || nameLimpio.includes("molida")) refPrice = 28.00;
    else if (nameLimpio.includes("pulpa de res") || nameLimpio.includes("pulpa")) refPrice = 42.00;
    else if (nameLimpio.includes("chuleta de res") || nameLimpio.includes("chuleta")) refPrice = 32.00;
    else if (nameLimpio.includes("costilla de res") || nameLimpio.includes("costilla")) refPrice = 26.00;
    else if (nameLimpio.includes("lomo de res") || nameLimpio.includes("lomo")) refPrice = 45.00;
    else if (nameLimpio.includes("chuleta de cerdo") || nameLimpio.includes("cerdo chuleta")) refPrice = 28.00;
    else if (nameLimpio.includes("costilla de cerdo") || nameLimpio.includes("lechón") || nameLimpio.includes("lechon")) refPrice = 30.00;
    else if (nameLimpio.includes("pierna de cerdo")) refPrice = 25.00;
    else if (nameLimpio.includes("trucha")) refPrice = 35.00;
    else if (nameLimpio.includes("sábalo") || nameLimpio.includes("sabalo")) refPrice = 25.00;
    else if (nameLimpio.includes("pejerrey")) refPrice = 40.00;
    else if (nameLimpio.includes("apio")) refPrice = 3.50;
    else if (nameLimpio.includes("choclo")) refPrice = 5.00;
    else if (nameLimpio.includes("tomate")) refPrice = 6.00;
    else if (nameLimpio.includes("zanahoria")) refPrice = 4.00;
    else if (nameLimpio.includes("cebolla")) refPrice = 5.00;
    else if (nameLimpio.includes("papa")) refPrice = 45.00;
    else if (nameLimpio.includes("lechuga")) refPrice = 3.00;
    else if (nameLimpio.includes("morron") || nameLimpio.includes("pimenton") || nameLimpio.includes("pimentón")) refPrice = 2.50;
    else if (nameLimpio.includes("arveja")) refPrice = 7.00;
    else if (nameLimpio.includes("espinaca")) refPrice = 3.50;
    else if (nameLimpio.includes("vainita")) refPrice = 5.00;
    else if (nameLimpio.includes("brocoli") || nameLimpio.includes("brócoli")) refPrice = 7.50;
    else if (nameLimpio.includes("locoto")) refPrice = 6.00;
    else if (nameLimpio.includes("camote")) refPrice = 5.00;
    else if (nameLimpio.includes("yuca")) refPrice = 6.00;
    else if (nameLimpio.includes("chuño") || nameLimpio.includes("chuñito")) refPrice = 46.00;
    else if (nameLimpio.includes("platano") || nameLimpio.includes("plátano") || nameLimpio.includes("banano")) refPrice = 4.00;
    else if (nameLimpio.includes("manzana")) refPrice = 2.00;
    else if (nameLimpio.includes("naranja")) refPrice = 0.80;
    else if (nameLimpio.includes("mandarina")) refPrice = 0.70;
    else if (nameLimpio.includes("papaya")) refPrice = 8.00;
    else if (nameLimpio.includes("frutilla")) refPrice = 15.00;
    else if (nameLimpio.includes("piña") || nameLimpio.includes("pina")) refPrice = 10.00;
    else if (nameLimpio.includes("palta")) refPrice = 5.00;
          ...
          {localProducts.map(product => {
            ...
            return (
              <div
                ...
                style={{ borderLeftWidth: "4px", borderLeftColor: alert.isOverpriced ? "#ef4444" : market.color }}
              >
                <div className="flex items-center justify-between">
                      ...
                      {isEditing ? (
                        ...
                        <>
                          <p className="text-sm text-gray-600">
                            Precio actual:{" "}
                            <span className="font-bold text-lg" style={{ color: market.color }}>
                  ...
                  <div className="flex gap-2 items-center">
                    {isEditing ? (
                      <>
                        <Button size="sm" onClick={() => handleSavePrice(product.id, product.name)} className="bg-green-600 hover:bg-green-700">
                          <Save className="w-4 h-4 mr-1" /> Guardar
                        </Button>
                        <Button size="sm" variant="outline" onClick={() => { setEditingProduct(null); setNewPrice(""); }}>
                          <X className="w-4 h-4" />
                        </Button>
                      </>
                    ) : (
                      <>
                        <Button
                          size="sm"
                          onClick={() => { setEditingProduct(product.id); setNewPrice(product.price.toString()); }}
